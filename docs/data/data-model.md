...
Документ описывает целевую модель данных API. Сейчас реализован только поиск адресов.

## Tag

Сущность, описывающая один физический **BinTag** двух типов: NFC-метка (`STANDARD`) или BLE-замок (`LOCK`). У тега всегда есть числовой первичный ключ `id`, человекочитаемая подпись `label` и тип `type`. В зависимости от `type` набор полей различается: для `STANDARD` хранится NFC-идентификатор, для `LOCK` - BLE параметры и ключи.
...
| type                | `'STANDARD'` \| `'LOCK'` | `tag_type`    | always        | on create      | `'LOCK'`                                                             |
| nfc_id              | `string` \| `null`       | `TEXT`        | STANDARD only | on create      | `'04:a2:b1:c2:d3:e4:80'`                                             |
...
| address_id          | `string` \| `null`       | `TEXT`        | optional      | on activation  | `'1168010100012340000000000'`                                       |
...
```sql
CREATE TYPE tag_type AS ENUM ('STANDARD', 'LOCK');
```

### Условия целостности:

1. Значение `type` ограничено `tag_type`:
   - `type` может быть только **STANDARD** или **LOCK**.

2. Взаимоисключающие поля по `type` (enforced через `CHECK`):
   - Если `type` = **STANDARD**, то:
...
5. Связи (рекомендовано):
   - `owner_id` (если задан) должен ссылаться на `users.id`.
   - `owner_id` должен совпадать с активным `TagAccess` с ролью **OWNER** (если `TagAccess` — источник прав).
   - `address_id` (если задан) указывает на `addresses.id` (soft-reference или FK по решению).

---

## TagAccess

Хранит выданные доступы к конкретному `Tag`. Это связь many-to-many между `tag` и `users`. Каждая строка означает: “`user_id` имеет доступ к `tag_id`”, с указанной ролью доступа.
...
1. `user_id` — FOREIGN KEY:
   - `user_id` обязан ссылаться на существующую запись `users.id`.
...
2. Уникальность активного email:
   ...
   - Если появятся другие провайдеры, уникальность должна быть по паре (`provider`, `provider_user_id`).
...
Примечание по связям: у таблицы `addresses` первичный ключ `id` типа `TEXT`, поэтому в `Tag` используется `address_id TEXT` как ссылка на `addresses.id` (soft-reference; при необходимости можно добавить FK).
