...
## Цель
Запустить и контролировать процесс загрузки адресных данных в PostgreSQL.

## Контекст
Импорт — отдельный CLI процесс. Он создаёт таблицы адресов, индексы и состояние импорта.

## Шаги
1. Убедитесь, что PostgreSQL доступен и переменные окружения настроены.
2. Запустите CLI импорт.
3. Проверьте логи и состояние таблиц.
...
## Режимы импорта
- **`upsert` (по умолчанию)**: вставка с `ON CONFLICT (id) DO UPDATE`.
- **`replace`**: вставка без `ON CONFLICT`, предполагается, что таблица очищена.

## Настройки импорта
Настройки берутся из переменных окружения `ADDRESS_*`. Полный список и дефолты — в [Переменные окружения](../reference/configuration/environment-variables.md).
...
- Импорт может занимать значительное время и диск.
- При параллельном запуске импорт будет пропущен (advisory lock).
