...
| Скрипт | Назначение | Когда использовать |
| --- | --- | --- |
| `npm run migration:run` | Запуск миграций TypeORM. | Первичная инициализация доменных таблиц. |
| `npm run migration:revert` | Откат последней миграции. | При необходимости отката схемы. |
