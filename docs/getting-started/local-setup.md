...
- `AUTH_*` параметры авторизации (секрет, TTL).
- `MAILER_*` параметры SMTP (для отправки OTP).
...
## Шаг 4. Миграции доменных таблиц

Перед импортом адресов создайте доменные таблицы (`users`, `auth_*`):

```bash
npm run migration:run
```

## Шаг 5. Импорт адресов
