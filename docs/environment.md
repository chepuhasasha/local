...
В репозитории есть отдельные шаблоны под окружения:

- `.env.development.example` — локальная разработка.
- `.env.test.example` — тесты и CI.
...
**Что это:** флаг расширенных полей на `/health/info`.

**На что влияет:**
- Если `true`/`1`, endpoint `/health/info` включает `env`, `version`, `commitSha`.

**Сценарии:**
- **Локально:** включайте для дебага.
- **Продакшн:** обычно `false`, чтобы не раскрывать лишнее.

### `APP_VERSION`

**Что это:** версия приложения (тег, билд-номер).

**На что влияет:**
- Отображается в `/health/info` при `HEALTH_VERBOSE=true`.

**Сценарии:**
- **Продакшн/стейдж:** проставляйте из CI/CD.

### `APP_COMMIT_SHA`

**Что это:** SHA коммита, на котором собран билд.
