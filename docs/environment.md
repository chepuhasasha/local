...
- Влияет на флаг `DB_SYNC`: синхронизация схемы разрешена только в `development`.

**Сценарии:**
- **Локальная разработка:** ставьте `development`, чтобы получать подробные логи и разрешать `DB_SYNC=true`.
...
**Что это:** флаг автосинхронизации схемы (`true`/`false`).

**На что влияет:**
- Если `true` и `NODE_ENV=development`, TypeORM может автоматически создавать/обновлять таблицы.
- В `production` и `test` синхронизация всегда отключена, даже если переменная выставлена.

**Сценарии:**
- **Локальная разработка:** `true`, чтобы быстрее поднять схему без миграций.
- **Продакшн:** всегда `false`, чтобы не повредить схему на боевых данных.
