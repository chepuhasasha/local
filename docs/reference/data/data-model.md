...
## Цель
Описать структуру данных, которые возвращает API поиска адресов (`/addresses/search`).

## Контекст
Модель соответствует текущему маппингу `AddressEntity` -> `AddressSearchResult`.

## Шаги
1. Используйте этот документ как контракт ответа `/addresses/search`.
2. При изменении структуры обновляйте и этот документ, и DTO.
...
## Ограничения
Документ описывает только ответ `/addresses/search`. Другие эндпоинты описаны отдельно.
