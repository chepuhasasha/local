...
## Цель
Описать, как сервис загружает и валидирует конфигурацию через `@nestjs/config`.

## Контекст
Конфигурация используется и в HTTP-приложении, и в CLI импорте.

## Шаги
1. Посмотрите, где подключается `ConfigModule`.
2. Используйте структуру `configuration()` для доступа к настройкам.
...
3. Обновите [переменные окружения](environment-variables.md).

## Ограничения
`ConfigService` не валидирует типы сам по себе — корректность гарантируется `validateEnv`.
