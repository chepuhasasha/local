...
- **OTP throttling**: ограничение частоты выдачи OTP для `/auth/email/start`.
- **Rate limiting**: глобальный throttling для публичных эндпоинтов с отдельными лимитами для auth-ручек.
- **Swagger в production**: UI отключён по умолчанию, включается только через `SWAGGER_ENABLED=true`.
